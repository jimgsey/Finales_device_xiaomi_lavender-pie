...
HELPER="${COS_ROOT}/vendor/cos/build/tools/extract_utils.sh"
if [ ! -f "${HELPER}" ]; then
...
BLOB_ROOT="$COS_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary
...
setup_vendor "${DEVICE}" "${VENDOR}" "${COS_ROOT}" false "${CLEAN_VENDOR}"
